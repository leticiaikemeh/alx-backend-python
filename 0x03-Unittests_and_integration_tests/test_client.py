...
class TestGithubOrgClient(unittest.TestCase):
    ...
    def test_org(self, test_org_name: str, expected: Dict, mock_get_json: Mock) -> None:
    ...
    def test_public_repos(self, mock_get_json: Mock) -> None:
        ...
        with patch('client.GithubOrgClient._public_repos_url', new_callable= PropertyMock) as mock_public_repos_url:
            mock_public_repos_url.return_value = "https://api.github.com/orgs/unakraft/repos"
            mock_get_json.return_value = [{"name": "repo1"}, {"name": "repo2"}]
            test_client = GithubOrgClient("unakraft")
            result = test_client.public_repos()
            self.assertEqual(result, ["repo1", "repo2"])
            mock_public_repos_url.assert_called_once()
            mock_get_json.assert_called_once_with("https://api.github.com/orgs/unakraft/repos")
    ...
    def test_has_license(self, repo: Dict[str, Dict], license_key: str, expected: bool) -> None:
        test_client = GithubOrgClient("test")
